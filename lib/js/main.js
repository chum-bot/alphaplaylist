...
function playlistToSelect() {
    ...
    .then(function(resp){
        ...
        for(i = 0; i < playlistArray.length; i++){
            listenerArray.push(document.getElementById(`picture${document.getElementsByClassName("pictures")[i].id.substr(-1)}`));
        }
        for(j = 0; j < listenerArray.length; j++){
           document.getElementById(listenerArray[j]).addEventListener("click", function(){return execute(listenerArray[j])});
        }
    })
}
    ...
    return gapi.client.youtube.playlistItems.list({
        ...
        .then(function (response) {
          ...
          let playlistList = document.getElementById("playlistDisplay");
            ...
            document.getElementById("moreSongsButton").style.display = "block";
            },
